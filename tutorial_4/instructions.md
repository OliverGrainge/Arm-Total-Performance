# Tutorial 4: Optimising a Redis Database with Arm Performix

A server can be fast on a small dataset and slow on a large one — and the reason is not always obvious from the code. [Redis](https://redis.io/) is one of the most widely deployed in-memory databases in the world, and it is already highly optimised. When its performance degrades under a realistic workload, the problem is usually in how the hardware handles memory, not in the application logic. Without hardware-level measurement, you would not know where to look.

This tutorial shows you how to use **Arm Performix** to diagnose and fix a memory bottleneck in a live Redis server. You'll populate Redis with a ~1 GB dataset of one million keys and run a throughput benchmark to establish a baseline. Then you'll attach Performix to the running server and use the **CPU Microarchitecture Analysis** recipe to identify where performance is being lost. A closer look at the **TLB Effectiveness** table will point you to a specific, actionable fix — one you can apply at the OS level without touching a line of Redis code.

You'll follow the same **profile → diagnose → fix → re-profile** loop from the earlier tutorials. By the end you'll know how to:

1. Attach Performix to a running process and profile a live server workload.
2. Use CPU Microarchitecture Analysis to identify a memory-bound bottleneck.
3. Read the TLB Effectiveness table to diagnose address-translation overhead.
4. Apply a kernel-level optimisation and confirm the improvement with Performix.

## What is Redis?

Redis stores data as simple key-value pairs: you give it a name (the key) and some data (the value), and it keeps everything in RAM so it can return results in microseconds. It is used everywhere:
...
## How paging works — and why it matters for Redis

Programs do not access physical RAM directly. Each program gets its own **virtual address space**, and the OS maps virtual addresses to physical locations in RAM by dividing memory into fixed-size blocks called **pages** (4 KB by default on Linux). The mapping is stored in a per-process **page table**.

Because the full page table lives in main memory, consulting it on every access would be slow. The CPU avoids this with a small hardware cache called the **Translation Lookaside Buffer (TLB)** that stores recent translations. A TLB **hit** is near-instant; a TLB **miss** forces a **page table walk** through memory that can cost tens to hundreds of cycles.

The TLB is small — the L1 DTLB on a typical Arm Neoverse core holds only a few entries. When a program's working set spans more pages than the TLB can track, accesses to evicted pages will trigger walks, and those walks add latency that is invisible to the application. The page size also matters: with 4 KB pages, each TLB entry covers very little memory, so a large dataset requires a large number of entries to be fully covered. Larger pages cover more memory per entry, which changes the arithmetic significantly. Linux supports both the default 4 KB pages and optional 2 MB "huge pages" — a distinction that will become important later in this tutorial.
...
The default `redis6` package on Amazon Linux 2023 does not include the benchmark tool and does not allow control over Transparent Huge Pages. Building `redis7` from source gives us both. Run the bash commands below to install and build it. 
...
Before measuring anything, set up a clean baseline with Redis running a default 4 KB page size.

### Disable Transparent Huge Pages

Ensure huge pages are turned off so the baseline uses the default small pages:
...
Start Redis with persistence disabled. We do not need Redis to save a backup of the data to disk. Disabling persistence also avoids `fork()` calls that would complicate profiling later:
...
- The L1 DTLB can only remember the locations of a **small number of pages** at a time
- Redis lookups are **random** — each request hashes to a different key, landing on a different page

Since the L1 DTLB can only track a few pages but the dataset spans 250,000 pages, random lookups frequently land on a page the TLB does not know about. We cannot easily fix the random access pattern (that is inherent to how a key-value store works), but we *can* reduce the number of pages the TLB needs to track — and that is something we can change at the OS level without modifying Redis itself.
...
- The L1 DTLB can only track a few at a time
